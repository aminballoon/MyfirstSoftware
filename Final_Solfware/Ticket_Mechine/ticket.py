...
class ticket:
    ...
    def input_even(self):
        ...
        db.collection(self.Branch).document('QueuePush').collection("ticket").document(new_id).set(
            {'Start_Time': -1, 'Status': -1, 'ID': new_id, 'Estimated_Time': 0, 'Queue_Time': -1, 'Wait_Time': 0,
             'No': new_queue, 'Stop_Time': -1, 'Type': str(self.counter_type[-1:]).upper()})
